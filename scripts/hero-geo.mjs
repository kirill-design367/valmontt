...
for (const [tag, w, h, m] of [
  ...
  // верхняя граница заголовка задана клиентом, нижний край блока не ближе 12 %
  const цель = m ? 0.30 : 0.56
  if (Math.abs(g.блокСверху - цель) > 0.01) {
    bad.push(`${tag}: верх блока на ${Math.round(g.блокСверху * 100)} % вместо ${Math.round(цель * 100)}`)
  }
  if (g.блокСнизу < 0.12) bad.push(`${tag}: под блоком ${Math.round(g.блокСнизу * 100)} % высоты, обещано не меньше 12`)
